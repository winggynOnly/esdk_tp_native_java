        ...
        setData: function (data)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (!p.columns)
                {
                    ...
                    $("table.l-table-nocheckbox", g.selectBox).append("<tr value='" + val + "'><td style='font-size:12px;' index='" + i + "' value='" + val + "' align='left'>" + txt + "</td>");
                } else
                {
...
//            var out = [];
            ...
            var ulLength=0;
            var action = p.action || "#";
            var method = p.method || "post";
            jform.attr("action",action);
            jform.attr("method",method);
            if (p.fields && p.fields.length)
            {
                ...
                $(p.fields).each(function (index, field)
                {
                    ...
                    {
                        out.push('<input type="hidden" id="' + id + '" name="' + name + '" value="' + value + '" />');
                        return;
                    }
                    ...
                    if (newLine)
                    {
                    	//为了计算form的宽度  add by taochunlong
                    	if (ulLength > formLength)
                    	{
                    		formLength = ulLength;
                    	}
                    	ulLength = 0;
                	...
                	var valiSpaceWidth = field.valiSpace || field.valiSpaceWidth || p.valiSpace;
                	var width = field.width || p.inputWidth;
                	var spaceWidth = field.space || field.spaceWidth || p.space;
                	
                	ulLength = ulLength +　labelWidth +　valiSpaceWidth　+　width + spaceWidth;
                   
                    //append label
                    out.push(g._buliderLabelContainer(field));
                    //append vali
                    out.push(g._buliderValiContainer(field));
                    //append input 
            ...
            {
            	var buts = $('<ul></ul>');
            	var li = $('<li style="width:' + formLength + 'px;text-align:center"></li>');
            	$(p.buttons).each(function (index,field)
            	{
            		var value = field.display;
            		var name = field.name;
            		var type = field.type || 'button';
            		var dfield = $('<input  type="' + type + '" class="l-button-button" name="' + name + '" value="' + value + '" />')
            		field.handler && dfield.click(function () {field.handler();});
            		li.append(dfield);
//            		item.click && ditem.click(function () { item.click(item); });
            ...
            jform.after('<iframe name="hidden_frame" id="hidden_frame" style="display:none"></iframe>');
            jform.attr('target','hidden_frame');
        },
        //标签部分
        ...
        //验证提示部分
        _buliderValiContainer: function (field)
        {
        	var g = this, p = this.options;
            var spaceWidth = field.valiSpace || field.valiSpaceWidth || p.valiSpace;
            var out = [];
            out.push('<li style="');
            if (spaceWidth)
            {
        ...
        _buliderControl: function (field)
        {
            ...
            {
            	out.push('<div><input onchange="$(\'#' + id + '-file\').val(this.value)" class="file" type="file" ');
            }	
            else
            {
            ...
            if (field.readOnly)
            {
            	out.push('readOnly="' + field.readOnly + '" ');
            }
            if (field.type)
            {
            ...
            if(field.type == "file")
            {
            	out.push("<input type='text' name='" + name + "-file' id='" + id + "-file'");
            	 //参数
                var fieldOptions = $.extend({
                    width: width - 18
                }, field.options || {});
                ...
                out.push(' />');
                out.push('<div class="l-file-icon"></div>');
            	out.push("</div>");
            }
            return out.join('');
        }
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        single : false,                       //在不是checkbox的情况下是否支持ctrl多选
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
            ...
            if (p.dataAction == "server")
            {
                if (p.usePager)
                {
                	var start = (p.newPage - 1)* p.pageSize;
                    param.push({ name: p.pageParmName, value: start });
        ...
        */
        ...
        singleton:function ()
        {
        	return this.options.single ? true : false;
        },
        //是否固定列
        ...
        */
        ...
        select: function (rowParm)
        {
            ...
            if (!g.enabledCheckbox() && (!g.ctrlKey || g.singleton())) //单选
            {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (!g.options.isLeaf(o))
                {
                    ...
                    if (p.parentIcon)
                    {
                        ...
                        treehtmlarr.push(g._getParentNodeClassName(g.hasChildren(o) ? true : false) + " ");
